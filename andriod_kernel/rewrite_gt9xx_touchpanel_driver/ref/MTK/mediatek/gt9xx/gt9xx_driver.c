...
reset_proc:
	...
	tp_power_on(1);
#if 0
	GTP_ERROR("turn on power vtouch\n");
	ret = regulator_enable(tpd->reg);
	if (ret != 0)
		TPD_DMESG("Failed to enable vtouch: %d\n", ret);
#endif
...
static int tpd_irq_registration(void)
{
	...
	node = of_find_matching_node(NULL, gt9xx_dt_match);
	if (node) {
...
void force_reset_guitar(void)
{
	...
	if (ret != 0)
		TPD_DMESG("Failed to disable vtouch: %d\n", ret);
#endif
...
static void gtp_pen_init(void)
{
    ...
    input_set_abs_params(pen_dev, ABS_MT_PRESSURE, 0, 1023, 0, 0);
...
static int tpd_local_init(void)
{
	...
	init_tp_power(tpd->tpd_dev, "vtouch");
	tp_power_on(1);
	...
	if (retval != 0) {
		TPD_DMESG("Failed to set vtouch voltage: %d\n", retval);
		return -1;
	}
#endif
