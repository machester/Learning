...
bool CONFIG_GTP_CHARGER_DETECT	= false;
bool CONFIG_GTP_ESD_PROTECT		= false;
bool CONFIG_GTP_SUPPORT_I2C_DMA = false;




#define GT9XX_DEV_NAME		"GT9XX"

#define HIGH				((int)1)
#define LOW					((int)0)

struct gt9xx_dts_info	{
	int init_gpio;
	int rst_gpio;
...
struct gt9xx_dts_info		*gt9xx_info;




static void tpd_pwr_on(void);
...
static struct tpd_driver_t tpd_device_driver = {
	.tpd_device_name = GT9XX_DEV_NAME,
...
static void tpd_pwr_on(void)
{
	if(!regulator_is_enabled(gt9xx_info->pwr_regulator)) {
		gt9xx_info->pwr_voltage = regulator_get_voltage(gt9xx_info->pwr_regulator);
		if(2800000 == gt9xx_info->pwr_voltage || 3300000 == gt9xx_info->pwr_voltage) {
			GTP_INFO("tp regulator on succeed, vol = %d uV", gt9xx_info->pwr_voltage);
		}
	}
	mdelay(10);
	gpio_set_value(gt9xx_info->rst_gpio, HIGH)
	mdelay(10);
	gpio_set_value(gt9xx_info->rst_gpio, HIGH)
	mdelay(20);
	gpio_set_value(gt9xx_info->rst_gpio, HIGH)
...
static int tpd_local_init(void)
{
	...
	if(IS_ERR(tpd->reg)) {
		GTP_ERROR("error: get vtouch regulator power node\n");
	}
...
{
	GTP_INFO("GT9xx series touch panel driver init");
...
{
	GTP_INFO("GT9xx series touch panel driver exit");
	tpd_driver_remove(&tpd_device_driver);
}
module_init(tpd_driver_init);
module_exit(tpd_driver_exit);
MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("GT9xx Series Touch Panel Driver");
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
	...
	if (!ret)
		...
		GTP_ERROR("error: in func:%s, ret = %d\n", __func__, ret);
			 ...
			 const struct i2c_device_id *id)
{
	...
	ret = regulator_enable(tpd->reg);
	if (ret != 0)
		TPD_DMESG("Failed to enable reg-vldo28: %d\n", ret);
	...
	GTP_INFO("tpd_int_gpio = %d, tpd_rst_gpio = %d\n", tpd_int_gpio, tpd_rst_gpio);
	regulator_vol_value = regulator_get_voltage(tpd->reg);
	if(regulator_vol_value > 0) {
		GTP_INFO("reg-vldo28 vol = %d uV", regulator_vol_value);
	}
